...
def pad_collate_fn(samples: List[List[int]]):
    ...
    MAX_LENGH=max([len(s) for s in samples])
    SEQ_SIZE=len(samples)
    output = torch.full([MAX_LENGH+1,SEQ_SIZE], fill_value=PAD_IX)
    for i in range(SEQ_SIZE):
        currentSequence = torch.cat((samples[i],torch.tensor([EOS_IX])))
        length_seq = currentSequence.size(0)
        output.narrow(0, 0, length_seq).select(1, i).copy_(currentSequence)
